...
import { IconPlus, IconBriefcase, IconUser, IconGrain } from '@tabler/icons';
...
const createProjectSchema = Yup.object().shape({
  ...
  project_type: Yup.string().required('Required field.'),
});
...
function Index(props) {
  ...
  const createProject = (values) => {
    api
      .post('/client/create-project/', JSON.stringify(values), {
    ...
    <div>
      ...
      <Box mt={25}>
        <Group position="right">
          {clientState.loading ? (
            <Skeleton visible height={35} width={100} />
          ) : (
            <Button
        ...
        {/* CARD */}
        {clientState.projects?.data &&
          clientState.projects.data.map((project) => (
