...
import { IconCircleCheck, IconCircleX, IconX, IconCheck } from '@tabler/icons';
...
function RegisterContent({ setSwitchForm }) {
  ...
  const checks = requirements.map((requirement, index) => (
    ...
    <PasswordRequirement
      ...
      meets={requirement.regex.test(formRegister.values.password)}
    />
  ));
  ...
  const registerUser = (values) => {
    setLoader(true);
    api
      .post('/user/register/', JSON.stringify(values))
      .then((res) => {
        setLoader(false);
    ...
    >
            ...
            >
              ...
              <Popover.Dropdown>
                ...
                <PasswordRequirement
                  ...
                  meets={formRegister.values.password.length > 5}
                />
                {checks}
              </Popover.Dropdown>
            </Popover>
              ...
              rightSection={
                item.name === 'email' ? (
                  formRegister.values.email === '' ? (
                    <IconCircleCheck color="grey" size={18} />
      ...
      <Group position="apart" spacing="xs">
        <Text
          ...
          onClick={() => setSwitchForm('login_form')}
        >
    ...
    <form onSubmit={formLogin.onSubmit(loginUser)}>
      ...
      <Group position="apart" spacing="xs">
        <Text
          ...
          color="gray"
          style={{ cursor: 'pointer' }}
          onClick={() => setSwitchForm('register_form')}
        >
  ...
  return (
    <Header height={HEADER_HEIGHT} mb={120} className={classes.root}>
      <Container className={classes.header}>
        ...
        <Modal
          opened={openForm}
          closefun={() => setSwitchForm('login_form')}
          setopened={setOpenForm}
          title={switchForm === 'login_form' ? 'Login' : 'Create an account'}
        >
          {switchForm === 'login_form' ? (
            <LoginContent setSwitchForm={setSwitchForm} />
          ) : (
            <RegisterContent setSwitchForm={setSwitchForm} />
          )}
        </Modal>
        <Burger
          opened={opened}
